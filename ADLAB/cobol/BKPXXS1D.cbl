      ...
      ******************************************************************
       IDENTIFICATION DIVISION.
        PROGRAM-ID. 'BKPXXS1D'.
        AUTHOR. WD4Z.
        INSTALLATION. 9.3.200.V20090814_0727.
        DATE-WRITTEN. 2/21/10 8:47 PM.
       DATA DIVISION.
      ...
      *          *********************************************
       ...
       2 PIC X(48) USAGE DISPLAY
           VALUE 'IRZ9999S Failed to retrieve the text of a Langua'.
       2 PIC X(48) USAGE DISPLAY
           VALUE 'ge Environment runtime message.  Check that the '.
       2 PIC X(48) USAGE DISPLAY
           VALUE 'Language Environment runtime message module for '.
       2 PIC X(47) USAGE DISPLAY
           VALUE 'facility IRZ is installed in DFHRPL or STEPLIB.'.
       LOCAL-STORAGE SECTION.
      ...
      * -------------------------------------------------------------
       ...
       GET-MESSAGE-TEXT.
           ...
           IF NOT CEE000 OF FEEDBACK-CODE AND
            ...
            MOVE LENGTH OF CONVERTER-ERROR-5
              TO ERROR-REASON-LENGTH
            MOVE CONVERTER-ERROR-5
              TO ERROR-REASON
           ELSE
      ...
      ******************************************************************
       ...
       3 PIC X(40) USAGE DISPLAY
           VALUE 'DFHCOMMAREA/link/link_comm/keynum/text()'.
       ...
       3 PIC X(48) USAGE DISPLAY
           VALUE 'DFHCOMMAREA/link/link_comm/server_request/text()'.
       ...
       2 X0000001E.
       3 PIC X(40) USAGE DISPLAY
           VALUE 'DFHCOMMAREA/link/link_comm/teamid/text()'.
       3 PIC X(8) USAGE DISPLAY
           VALUE SPACES.
       ...
       3 PIC X VALUE '-'.
       2 X00000020.
       3 PIC X(46) USAGE DISPLAY
           VALUE 'DFHCOMMAREA/link/link_comm/user_request/text()'.
       ...
       2 XPATH-HASH-TABLE-ENTRIES OCCURS 17 TIMES.
       3 XPATH-TEXT PIC X(48).
       ...
       2 CONVERTER-ERROR-3.
       3 PIC X(48) USAGE DISPLAY
           VALUE 'IRZ9999S Failed to register a Language Environme'.
       3 PIC X(21) USAGE DISPLAY
           VALUE 'nt exception handler.'.
       2 CONVERTER-ERROR-4.
       3 PIC X(48) USAGE DISPLAY
           VALUE 'IRZ9999S Failed to unregister a Language Environ'.
       3 PIC X(23) USAGE DISPLAY
           VALUE 'ment exception handler.'.
       2 CONVERTER-ERROR-7.
       3 PIC X(40) USAGE DISPLAY
           VALUE 'Language Environment Service Call Failed'.
       2 CONVERTER-ERROR-8.
       ...
       1 HASH-TOKEN PIC X(48).
       1 REDEFINES HASH-TOKEN.
       2 OCCURS 12 TIMES INDEXED BY HASH-DIGIT-NDX.
      ...
      * 01 DFHCOMMAREA.
       ...
       HASH-XPATH.
           ...
           END-PERFORM
           DIVIDE HASH-VALUE BY 17 GIVING HASH-DISCARD
            REMAINDER HASH-VALUE
           END-DIVIDE
           IF HASH-VALUE LESS THAN ZERO
            ADD 17 TO HASH-VALUE
           ELSE
            ADD 1 TO HASH-VALUE
           END-IF
           .
       ...
       X00000042.
           MOVE CHAR-CON (1:1) TO X00000037 X00000031
           INSPECT X00000031 REPLACING ALL
            '0' BY '9' '1' BY '9' '2' BY '9' '3' BY '9' '4' BY '9'
            '5' BY '9' '6' BY '9' '7' BY '9' '8' BY '9' ' ' BY '?'
           MOVE X00000031 TO X00000030
           GO TO LOOKUP-SOURCE
           .
       X00000043.
           MOVE CHAR-CON (1:2) TO X00000038 X00000032
           INSPECT X00000032 REPLACING ALL
            '0' BY '9' '1' BY '9' '2' BY '9' '3' BY '9' '4' BY '9'
            '5' BY '9' '6' BY '9' '7' BY '9' '8' BY '9' ' ' BY '?'
           MOVE X00000032 TO X00000030
           GO TO LOOKUP-SOURCE
           .
       X00000044.
           MOVE CHAR-CON (1:3) TO X00000039 X00000033
           INSPECT X00000033 REPLACING ALL
            '0' BY '9' '1' BY '9' '2' BY '9' '3' BY '9' '4' BY '9'
            '5' BY '9' '6' BY '9' '7' BY '9' '8' BY '9' ' ' BY '?'
           MOVE X00000033 TO X00000030
           GO TO LOOKUP-SOURCE
           .
       X00000045.
           MOVE CHAR-CON (1:4) TO X0000003A X00000034
           INSPECT X00000034 REPLACING ALL
            '0' BY '9' '1' BY '9' '2' BY '9' '3' BY '9' '4' BY '9'
            '5' BY '9' '6' BY '9' '7' BY '9' '8' BY '9' ' ' BY '?'
           MOVE X00000034 TO X00000030
           GO TO LOOKUP-SOURCE
           .
       X00000046.
           MOVE CHAR-CON (1:5) TO X0000003B X00000035
           INSPECT X00000035 REPLACING ALL
            '0' BY '9' '1' BY '9' '2' BY '9' '3' BY '9' '4' BY '9'
            '5' BY '9' '6' BY '9' '7' BY '9' '8' BY '9' ' ' BY '?'
           MOVE X00000035 TO X00000030
           GO TO LOOKUP-SOURCE
           .
       X00000047.
           MOVE CHAR-CON (1:6) TO X0000003C X00000036
           INSPECT X00000036 REPLACING ALL
            '0' BY '9' '1' BY '9' '2' BY '9' '3' BY '9' '4' BY '9'
            '5' BY '9' '6' BY '9' '7' BY '9' '8' BY '9' ' ' BY '?'
           MOVE X00000036 TO X00000030
           GO TO LOOKUP-SOURCE
           .
       LOOKUP-SOURCE.
           SEARCH ALL NPSA
       ...
       SIGNAL-CONDITION.
           IF OTHER-ERROR = 'N'
            ...
            MOVE 'BKPXXS1I' TO CEECMI-DATA (1:8)
      ...
      ******************************************************************
       ...
       1 CONVERTER-ERROR-6.
       2 PIC X(48) USAGE DISPLAY
           VALUE 'IRZ9999S Failed to resume execution after catchi'.
       2 PIC X(36) USAGE DISPLAY
           VALUE 'ng a Language Environment exception.'.
       LOCAL-STORAGE SECTION.
       1 FEEDBACK-CODE.
       2 CONDITION-TOKEN-VALUE.
       88 CEE000 VALUE X'0000000000000000'.
      ...
      ******************************************************************
       ...
       2 PIC X(48) USAGE DISPLAY
           VALUE 'DFHCOMMAREA xmlns="http://www.BKPXXS1O.com/schem'.
       2 PIC X(21) USAGE DISPLAY
           VALUE 'as/BKPXXS1OInterface"'.
       1 X00000001.
